...
TEST_CASES = (
    TestCase(
        ...
        expected_intent="business_seller_info",
    ),
    TestCase(
        name="단순 변심 반품",
        question="단순 변심으로도 반품할 수 있나요?",
        required_terms=("7일 이내", "반품", "소비자가 부담"),
        expected_intent="change_of_mind_return",
    ),
    TestCase(
        name="상품 불일치 반품 기한",
    ...
    TestCase(
        ...
        required_terms=("방해해서는 안 됩니다", "반품 요청 내역"),
        expected_intent="return_obstruction",
    ),
    TestCase(
        name="변형 - 마음이 바뀐 반품",
    ...
    TestCase(
        ...
        required_terms=("방해해서는 안 됩니다", "상담 기록"),
        expected_intent="return_obstruction",
    ),
    TestCase(
        name="변형 - 입점 판매자 정보",
    ...
    TestCase(
        ...
        required_terms=("택배비는 소비자가 부담", "판매자가 반환 비용을 부담"),
        expected_intent="return_cost",
    ),
    TestCase(
        name="변형 - 품절 결제금 환급",
...
def run_test(test_case: TestCase) -> tuple[bool, list[str]]:
    ...
    if (
        ...
        errors.append(
            "의도 불일치: "
            f"기대={test_case.expected_intent}, 실제={intent}"
        )
...
def main() -> int:
    ...
    for index, test_case in enumerate(TEST_CASES, start=1):
        ...
        try:
            ...
            success = False
            errors = [f"실행 오류: {type(exc).__name__}: {exc}"]
    ...
    if failed:
        print("회귀 테스트 실패 항목을 확인해주세요.")
        return 1

    print("모든 회귀 테스트를 통과했습니다.")
    return 0
