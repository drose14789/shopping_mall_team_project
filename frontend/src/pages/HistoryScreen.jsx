...
export default function HistoryScreen({ setScreen, setSelectedFile }) {
  ...
  const historyPageCount = Math.max(
    1,
    Math.ceil(historyData.length / HISTORY_PAGE_SIZE)
  );
  ...
  const stats = useMemo(() => {
    return [
      {
        label: "총 분석 회차",
        value: `${historyData.length}회`,
        sub: "서버 저장 기준",
      },
      {
        label: "최근 분석 기간",
        value: latestHistory
          ? `${latestHistory.analysis_start_time}~${latestHistory.analysis_end_time}`
          : "-",
        sub: latestHistory ? `${latestHistory.quarter} 시즌` : "분석 이력 없음",
      },
      {
        label: "최근 분석일",
        value: latestHistory?.created_at
          ? latestHistory.created_at.slice(0, 10)
          : "-",
        sub: latestHistory?.file_name || "업로드된 분석 파일 없음",
      },
    ];
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-slate-50 p-6 space-y-4">
      <div>
        <h2 className="text-xl font-bold text-slate-800">분석 이력</h2>
      ...
      <div className="bg-white rounded-xl border border-slate-100 overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
          <h3 className="font-semibold text-slate-800 text-sm">분석 목록</h3>
        ...
        {historyData.length === 0 ? (
          <div className="px-5 py-12 text-center">
            ...
            <p className="text-xs text-slate-400 mt-1">
              상품 성과 파일을 업로드하고 분석을 완료하면 이곳에 이력이
              표시됩니다.
            </p>
            ...
            <button
              type="button"
              onClick={() => setScreen("upload")}
              className="mt-5 px-4 py-2 rounded-lg bg-blue-600 text-white text-xs font-semibold hover:bg-blue-700 transition"
            >
              파일 업로드하러 가기
            </button>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-slate-50">
                  {[
                    ...
                    <th
                      key={h}
                      className="text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wide px-5 py-3 whitespace-nowrap"
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody className="divide-y divide-slate-50">
                {pagedHistory.map((row) => {
                  ...
                  return (
                    <tr
                      key={`${row.file_name}-${row.created_at}`}
                    ...
                    >
                      ...
                      <td className="px-5 py-3.5 text-xs text-slate-500">
                        {row.analysis_start_time}~{row.analysis_end_time}
                      </td>

                      <td className="px-5 py-3.5">
                        <span
                          className="inline-flex items-center gap-1 text-xs px-2 py-1 rounded-md border font-semibold whitespace-nowrap"
                          style={{
                      ...
                      <td className="px-5 py-3.5">
                        <button
                          onClick={() => {
                            setSelectedFile(row.file_name);
                            setScreen("results");
                          }}
                          className="text-xs font-semibold text-blue-600 hover:text-blue-800 hover:underline transition flex items-center gap-1 whitespace-nowrap"
                        >
                          결과 보기
                          <svg
                            width="11"
              ...
              <div className="flex items-center justify-between px-5 py-3 border-t border-slate-100 bg-slate-50/50">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-[11px] font-semibold border transition ${
                      historyPage >= historyPageCount - 1
                        ? "text-slate-300 border-slate-150 cursor-not-allowed"
                        : "text-slate-600 border-slate-200 hover:bg-slate-100"
                    }`}
                  >
